...
class GamePage extends Component {
  ...
  playAgain = () => {
    this.startTimer()
    this.setState({second: 60, score: 0, gameOver: false})
  }
  ...
  getScore = imageUrl => {
    ...
    const isSame = imageUrl === imagesList[index].imageUrl
    if (isSame) {
      const i = Math.floor(Math.random() * 29)
      this.setState(prevState => ({score: prevState.score + 1, index: i}))
    } else {
      clearInterval(this.timerID)
      this.setState({gameOver: true})
    }
  ...
  render() {
    const {activeTabId, score, index, second, gameOver} = this.state
    const filteredImages = this.getFilteredImages()
    const url = imagesList[index].imageUrl
    return (
      <div className="bg-container">
        <NavBar score={score} second={second} />
